...
## 9. Ajustes desta rodada (pedido seguinte)

| Pedido | O que foi feito |
|---|---|
| Gráfico de pizza faltando (Faturamento/Lucro/Impostos) | O gráfico já existia mas ficava ambíguo (só mostrava Lucro vs Impostos). Virou um **anel (doughnut)** com o valor do **Faturamento escrito no centro** e o anel dividido em Lucro/Impostos — as 3 métricas ficam visíveis ao mesmo tempo, sem contar valor em dobro |
| Aba de Caixa / Fluxo de Caixa | Nova página `caixa.html` + `assets/js/caixa.js` + migration `003_fluxo_caixa.sql`. Toda venda/serviço lançado no Painel vira **entrada** automática; todo imposto calculado vira **saída** automática ("a reservar"); o usuário lança manualmente aportes, retiradas e despesas. Tem saldo em caixa, gráfico de saldo acumulado no mês e tabela unificada (automático + manual) |
| Interface mais bonita | `style.css`: cards de estatística ganharam uma barra de cor lateral com significado (verde=positivo, terracota=saída/imposto, dourado=neutro); sidebar com indicador de página atual mais claro; sombra sutil e leve elevação nos cards; fundo com textura sutil (gradientes muito discretos, mantendo o papel como base); números de tabela alinhados à direita (leitura financeira); modais com uma pequena animação de entrada |

### Banco: rodar mais uma migration

```
supabase/migrations/003_fluxo_caixa.sql
```

Roda depois da 002. Cria a tabela `movimentos_caixa`, as triggers que sincronizam automaticamente com `transacoes` e `impostos_calculados`, e preenche o histórico de quem já tinha lançamentos (para o saldo não nascer errado).

Nenhuma Edge Function nova, nenhum segredo novo — é só front-end + banco.
